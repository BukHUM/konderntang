...
if ('modal' === $switcher_style):
    ...
    <div class="konderntang-language-modal" id="konderntang-language-modal-<?php echo esc_attr($unique_id); ?>"
        style="display: none;" role="dialog" aria-modal="true"
        aria-labelledby="modal-title-<?php echo esc_attr($unique_id); ?>">
        ...
        <div class="konderntang-language-modal-container">
            ...
            <div class="konderntang-language-modal-header">
                ...
                <button type="button" class="konderntang-language-modal-close" data-close-modal
                    aria-label="<?php esc_attr_e('ปิด', 'konderntang'); ?>">
                    <i class="ph ph-x"></i>
                </button>
            </div>

            <!-- Geo-location suggestion banner (hidden by default, shown via JS) -->
            <div class="konderntang-language-suggestion" id="language-suggestion-<?php echo esc_attr($unique_id); ?>"
                style="display: none;">
                ...
                <a href="#" class="konderntang-language-suggestion-btn"
                    id="use-suggested-lang-<?php echo esc_attr($unique_id); ?>">
            ...
            <?php if ($show_search && count($languages) > 4): ?>
                ...
                <div class="konderntang-language-modal-search">
                    ...
                    <input type="text" id="konderntang-language-search-<?php echo esc_attr($unique_id); ?>"
                        placeholder="<?php esc_attr_e('ค้นหาภาษา...', 'konderntang'); ?>"
                        class="konderntang-language-search-input" autocomplete="off" />
                </div>
            <?php endif; ?>

            <!-- Language Grid -->
            <div class="konderntang-language-modal-grid"
                id="konderntang-language-list-<?php echo esc_attr($unique_id); ?>">
    ...
    <div class="konderntang-language-switcher <?php echo $is_mobile ? 'is-mobile' : ''; ?>">
        <button type="button" class="konderntang-language-button"
            id="konderntang-language-button-<?php echo esc_attr($unique_id); ?>" aria-haspopup="dialog">
            <?php if ($current_lang_data && $show_flags): ?>
                <span
                    class="konderntang-language-flag"><?php echo esc_html(konderntang_get_lang_flag($current_lang_data)); ?></span>
            <?php endif; ?>
            <span
                class="konderntang-language-name"><?php echo $current_lang_data ? esc_html($current_lang_data['name']) : esc_html__('ภาษา', 'konderntang'); ?></span>
...
<?php else: ?>
    ...
    <div
        class="konderntang-language-switcher konderntang-language-switcher-dropdown <?php echo $is_mobile ? 'is-mobile' : ''; ?>">
        <button type="button" class="konderntang-language-button"
            id="konderntang-language-button-<?php echo esc_attr($unique_id); ?>" aria-haspopup="listbox"
            aria-expanded="false">
            <?php if ($current_lang_data && $show_flags): ?>
                <span
                    class="konderntang-language-flag"><?php echo esc_html(konderntang_get_lang_flag($current_lang_data)); ?></span>
        ...
        <div class="konderntang-language-dropdown" id="konderntang-language-dropdown-<?php echo esc_attr($unique_id); ?>"
            role="listbox">
            <?php foreach ($languages as $lang):
                $flag = konderntang_get_lang_flag($lang);
                $is_current = !empty($lang['current_lang']);
                ?>
                <a href="<?php echo esc_url($lang['url']); ?>"
                    class="konderntang-language-item <?php echo $is_current ? 'current' : ''; ?>" role="option"
